...
|--------------|-------------------------------------|-------------------------------------------------------------|
| Frontend     | React 19 + Vite + Recharts          | Fast HMR, rich charting ecosystem                           |
...
- **TCP/IP Socket simulation** — A Python or C# script mimics a medical device sending byte streams to validate hardware-software integration.
  - *Cloud caveat:* Free-tier PaaS exposes only ports 80/443. Use WebSocket simulation in the cloud; keep raw TCP for local/demo.
...
**Why this works:** You get a managed cloud database while maintaining full control over backend business logic, complex queries, and SignalR socket routing in .NET.

### 4.1 CLI & Migrations
To ensure reproducible database states across different environments:
...
- **Supabase Pause Caveat:** Free Supabase projects pause after 1 week of inactivity. You will need to log into the Supabase dashboard and click "Restore" to resume the project.

---

React is the chosen framework for this project's requirements:

| Factor                  | React + Vite          | Vue 3 + Vite        |
|-------------------------|-----------------------|---------------------|
| Ecosystem & libraries   | Largest               | Sufficient          |
| Real-time chart perf    | Good (Recharts)       | Slightly smoother   |
| State Management        | Robust                | Flexible            |
...
3. **Tech choice justifications** — *"Used Supabase purely as a managed PostgreSQL instance to leverage JSONB indexing while keeping business logic and WebSocket routing strictly inside .NET 8."*
...
- **Swashbuckle**: Automatically generates OpenAPI/Swagger documentation for the API.
...
- **Grafana**: The industry standard for visualization. Pre-provisioned datasources ensure that the project is "plug-and-play" for the user.