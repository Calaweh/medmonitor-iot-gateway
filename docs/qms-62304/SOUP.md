...
| **Hangfire** | 1.8.x | Background Jobs | Medium risk. Risk of silent job failure (retention/overdue checks). Mitigated by monitoring the `/hangfire` dashboard, explicit `LogError` on job failure, and retry policies. |
